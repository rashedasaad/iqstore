    ...
    <div class="allInvoice">
        <div class="continer">
            ...
            <div class="invoice">
                ...
                <h1 style="margin-bottom:20px; text-align: center;">{{ $info['title'] }}</h1>
                <h4 style="margin-bottom:20px ;">your invoice refound: {{ $info['refund'] }}</h4>
                <h4 style="margin-bottom:20px ;">your telefone nummber: {{ $info['telefone'] }}</h4>
                <h4 style="margin-bottom:20px ;">your email: {{ $info['email'] }}</h4>
                <p>{{ $info['message'] }}
                    <br>
                    <br>
                    <?php $orders = $info['order']; ?>
                    @foreach ($orders as $order)
                        <span>order: {{$order['name']}}, price: {{ $order['price'] }}, qtn: {{ $order['qtn'] }}</span>
                        <br>
                    @endforeach
